...
## 19. k3 偶发"run 被接受但永不产出"，两次复现确认为上游 flake

**现象**：k3 场景偶发 `run_accepted` 之后没有正文、没有终止事件；服务端侧
`GET /runs/{id}` 显示 `turns: 0`——run 创建了，但 agent 从未产出任何内容。

**复现**（2026-09-14，两次连续全量场景复跑）：

| 日期 | 通过/失败 | k3 表现 |
| ---- | --------- | ------- |
| 前次 | 17 / 3    | 卡住（未定论） |
| 本次 | 18 / 2    | 再次卡住：`{"turns":0}` |

deepseek-v4-flash 同一轮 153 帧流式全过、工具与错误态场景全过，排除客户端链路问题。

**判定**：上游（Kimi 通道）偶发，与客户端无关。客户端已按第 13 条的要求兜住：
`classify()` 把"服务端 turns:0"识别为真卡住并单独报，且界面能显示 run 失败。
不要因为这一条失败就把 API 场景当红的——跑之前先确认 k3 不在卡住窗口里。
