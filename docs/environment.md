...
| 工具审批（含兜底选项）                    | ✅          | ✅                      | ✅                            |
| `ask_user` 提问与作答 / 取消              | ✅          | ✅                      | ✅                            |
| 上下文用量 / cache 统计（`GET …/status`） | ✅          | ✅（session-info 面板） | ✅（点聊天标题打开）          |
...
**`/status` 只给了一半，界面就得只显示一半**：这台部署的 `/status` 返回
`message_count` / `used_tokens` / `cache_stats`，但**没有 `context_window`**，
模型配置里也没有。于是会话信息面板只显示绝对值，**不显示百分比**——没有分母的
百分比是编出来的，而它恰恰会被用来判断"还有多少余量"。

上游较新的版本会同时给 `context_window` 与 `budget_plan`，那时同一套代码自动开始
显示进度条与百分比（`features/session/sessionInfo.ts` 定了这条规则，两个场景
`chat-info` / `chat-info-with-window` 分别把两种形状渲染出来）。验收的
`chat-info`（live 批次）会**直查服务端再和屏幕上的数对照**，并断言没有窗口时
不出现百分比；服务端哪天给了窗口，这条断言会失败一次，那时把它改成"必须有百分比"。
